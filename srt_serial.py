...
class PythonSerialDriver():
    ...
    def moveTo(self, where = None, pressure = 0, delay = None, bRtFeedback = False, bRlfReset = True):
        if where is None:
            where = self.FLG_ZERO 
        if delay is None:
            delay = self.DELAY             
    ...
    def move3Fingers(self, bOpen = True, pressure = 0, delay = 0):      
        if pressure == 0:
    ...
    def open(self, bOpen = True):      
        if bOpen is True:
            return self.move3Fingers(True, 70)
        else:
